...
public class AE01_vistaCorregido {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AE01_vistaCorregido window = new AE01_vistaCorregido();
	...
	public static String ultimaModificacion(File rutaArchivo) {
		...
		try {
			...
			tiempo = formato.format(lastModifiedTime.toMillis());
		} catch (Exception err) {
			err.printStackTrace();
		}
		return tiempo;
				...
				if (rutaDirectorio1.isEmpty()) {
					...
					try (BufferedWriter bw = new BufferedWriter(new FileWriter("ruta1.txt"))) {
						...
						for (int i = 0; i < listaArchivos.length; i++) {
							File ficheroTemporal = new File(
									FileDirectorio1.getAbsolutePath() + File.separator + listaArchivos[i]);
							...
							if (ficheroTemporal.isDirectory()) {
								bw.write("\\" + listaArchivos[i] + " | " + tamaño + " | " + ultimaModificacion);
							} else {
								if (listaArchivos[i].length() < 15) {
									bw.write(listaArchivos[i] + " | " + tamaño + " | " + ultimaModificacion);
								} else {
									String nuevoNombreFile15 = listaArchivos[i].substring(0, 12) + "(...)";
									bw.write(nuevoNombreFile15 + " | " + tamaño + " | " + ultimaModificacion);
								}
							}
							bw.newLine();
						}
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (rutaDirectorio2.isEmpty()) {
					...
					try (BufferedWriter bw = new BufferedWriter(new FileWriter("ruta2.txt"))) {
						...
						for (int i = 0; i < listaArchivos.length; i++) {
							File ficheroTemporal = new File(
									FileDirectorio2.getAbsolutePath() + File.separator + listaArchivos[i]);
							String tamaño = calcularTamaño(ficheroTemporal);
							String ultimaModificacion = ultimaModificacion(ficheroTemporal);
